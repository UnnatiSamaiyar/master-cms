...
export default function RemoveAssignedAdmins({ admins }: Props) {
    ...
    <Fragment>
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-3xl font-bold">
          Select admins to remove from "{searchParmas.get("name")}" website.
        </h1>
      ...
      {/* Conditional rendering for no admins */}
      {admins.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          <p>No admins assigned to "{searchParmas.get("name")}" website.</p>
        </div>
      ) : (
        <Fragment>
          ...
          {/* Submit button (disabled if no admins are selected) */}
          <SubmitButton
            isPending={isPending}
            className="w-fit"
