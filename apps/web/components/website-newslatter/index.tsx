...
const WebsiteNewsletter = ({ websites }: Props) => {
  ...
  useEffect(() => {
    ...
    const fetchnewsletters = async () => {
      try {
        ...
        const url = `/api/website-newsletter/website/${selectedWebsite.id}?page=${page}&perRow=${perRow}&search=${search}`;
        ...
        <div className="max-w-md w-full bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 mt-1">
              Choose a website to manage its newsletters
            </p>
          </div>
          {/* Search Bar */}
          <div className="mb-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Manage Newsletters for {selectedWebsite?.name}
          </p>
        </div>
      ...
      {!isLoading && (
        <>
          {newsletters.length < 1 ? (
            <div className="text-center my-10">No newsletters found</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
