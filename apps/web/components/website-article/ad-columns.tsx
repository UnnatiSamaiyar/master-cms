...
import { ChangeArticleAdsForm } from "./change-sidebar";
import DeleteWebsiteArticleAds from "./delete-ads";
import Link from "next/link";
import { useParams, useSearchParams } from "next/navigation";

export const WebsiteArticleAdsColumns: ColumnDef<AdType>[] = [
  {
    accessorKey: "id",
    header: "Id",
  },
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const websiteId = useSearchParams().get("websiteId");
      const copyMessageId = () => {
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem className="flex w-full cursor-pointer">
              <Link
                className="w-full"
                href={`/dashboard/website-article/${id}/ads/${ad.id}?websiteId=${websiteId}`}
              >
                Edit Ad
              </Link>
            </DropdownMenuItem>
            <DeleteWebsiteArticleAds ad={ad} />
          </DropdownMenuContent>
        </DropdownMenu>
      );
    },
