...
export function CreateWebsiteArticleAdsForm() {
  ...
  function onSubmit(data: adsFormValue) {
    if (!file) return toast.info("Please upload image", toastOptions);
    startTransition(async () => {
      const formData = new FormData();
      formData.set("title", data.title);
      formData.set("targetUrl", data.targetUrl);
      formData.set("articleId", articleId as string);
      ...
      const response = await fetch(
        `${BASE_URL}/api/website-ads/push/articles/ads/${websiteId}`,
        {
          method: "POST",
          headers: {
